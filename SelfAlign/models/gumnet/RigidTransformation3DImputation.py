...
from tensorflow.compat.v1.spectral import fft, ifft
...
def angles_to_rotation_matrix(theta_z1, theta_y, theta_z2):
    Rz1 = tf.reshape([
...
class RigidTransformation3DImputation(Layer):
    ...
    def __init__(self, output_size, padding_method="replicate", **kwargs):
    ...
    def _rescale_theta(self, tensors):
        theta, X = tensors
        shape = tf.cast(tf.shape(X)[0:3], dtype=tf.float32)
        shift = 0.5
    ...
    def _rotation_matrix_zyz(self, params):
        ...
        rm = angles_to_rotation_matrix(phi, theta, psi_t)
    ...
    def _interpolate3d(self, imgs, x, y, z):
        ...
        xlen_f = tf.cast(xlen, dtype=tf.float32)
        ylen_f = tf.cast(ylen, dtype=tf.float32)
        zlen_f = tf.cast(zlen, dtype=tf.float32)
        ...
        dx = x - tf.cast(x0, dtype=tf.float32)
        dy = y - tf.cast(y0, dtype=tf.float32)
        dz = z - tf.cast(z0, dtype=tf.float32)
    ...
    def _mgrid(self, *args, **kwargs):
        ...
        high = tf.cast(high, dtype=tf.float32)
