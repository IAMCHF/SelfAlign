...
def get_cubes_list(iteration_count):
    ...
    num_train = int(100 * 0.9)
    num_valid = int(100 * 0.1)
...
def predict(iter_count):
    ...
    # predict_txt_path = "/HBV/Caohaofan/test_data/100-snr100.txt"
...
def predict_and_save_data(mrc_paths: List[str], template_mrc_paths: List[str], batch_size: int, model,
    ...
    iter_result_dir = f'{result_dir}/iter_{iter_count - 1}'
...
result_dir = "/HBV/Caohaofan/result_iter20"
data_dir = "/HBV/Caohaofan/result_iter20/data"
if os.path.exists(result_dir):
    shutil.rmtree(result_dir)
if not os.path.exists(data_dir):
    os.makedirs(data_dir)

src_file = "/HBV/Caohaofan/test_data/wedge/wedge_snr01.txt"
predict_txt_path = "/HBV/Caohaofan/test_data/wedge/wedge_snr01.txt"
...
for i in range(times):
    if i == 0:
        logging.info("prepare_first_model")
        init_model_name = os.path.join(result_dir, 'model_iter00.h5')
        shutil.copy("/HBV/Caohaofan/iter50_slefalign_nowedge_snr001/model_iter49.h5", init_model_name)
        logging.info("Before finetuning:")
    else:
        predict(i)
        init_model = "{}/model_iter{:0>2d}.h5".format(result_dir, i - 1)
        get_cubes_list(i)
        train_pcrnet(i, init_model, int(100 * 0.9) // 64, 64, ln_rate)
        ln_rate *= 0.9
        if 2 < i < times-1:
            os.remove("{}/model_iter{:0>2d}.h5".format(result_dir, i - 2))
    if i == times - 1:
        predict(times)
