...
theta_range = np.linspace(np.pi / 1000, np.pi / 6, num=90, endpoint=False)
phi_range = np.linspace(np.pi / 1000, 2 * np.pi / 6, num=180, endpoint=False)
...
#     batch_indices = []
#     np.random.seed(42)
#     for _ in range(16200):
#         theta = np.random.uniform(low=np.pi / 1000, high=np.pi)
#         phi = np.random.uniform(low=np.pi / 1000, high=2 * np.pi)
