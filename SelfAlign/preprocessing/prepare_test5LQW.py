...
def get_cubes(inp, rota_folder):
    ...
    np.random.seed(43)
    for _ in range(100):
        theta = np.random.uniform(low=np.pi / 1000, high=np.pi / 6)
        phi = np.random.uniform(low=np.pi / 1000, high=2 * np.pi / 6)
...
def get_cubes_list():
    ...
    for i, mrc in enumerate(all_mrc_list):
        inp.append((mrc, int(i * 100)))
    if not os.path.exists(rota):
        os.makedirs(rota)
    rota_folder = rota
    for i in inp:
        get_cubes(i, rota_folder)
    train_outfile = '{}/5LQW.txt'.format(data_dir)
...
"test_data, test_data_067, test_data_133, test_data_2, test_data_random, test_data_rotation_strategy"

data_dir = "/newdata3/chf/test_data_rotation_strategy" + "/snr001"
rota = data_dir + "/5LQW"
