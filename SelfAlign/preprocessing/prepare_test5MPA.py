...
def get_cubes(inp, rota_folder):
    ...
    np.random.seed(43)
    for _ in range(100):
        theta = np.random.uniform(low=np.pi / 1000, high=np.pi / 6.0)
        phi = np.random.uniform(low=np.pi / 1000, high=2 * np.pi / 6.0)
...
def get_cubes_list():
    ...
    train_outfile = '{}/5MPA.txt'.format(data_dir)
...
rota = data_dir + "/5MPA"
subtomo_star = "/HBV/Caohaofan/selfalign/5MPA.star"
